...
int main( )
{
    ...
    for ( unsigned int testCase = initialTestCase; testCase < 6; testCase++ )
    {
        ...
        const bool readAerodynamicCoefficientsFromFile = false;
                ...
                if ( !( propagatorType == 1 || propagatorType == 2 || propagatorType == 3 ) )
                {
                    ...
                    for ( unsigned int integratorType = 0; integratorType < integratorLimit; integratorType++ )
                    {
                        ...
                        for ( unsigned int value = 0; value < valueLimit; value++ )
                        {
                            ...
                            std::map< double, unsigned int > functionEvaluationsMap =
                                    dynamicsSimulator->getCumulativeNumberOfFunctionEvaluations( );
                            ...
                            {
                                // Retrieve current Cartesian state and convert to Keplerian elements
